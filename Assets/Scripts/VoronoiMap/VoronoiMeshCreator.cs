    ...
    public class VoronoiMeshCreator : MonoBehaviour
    {
        ...
        public void GenerateMesh(VoronoiMapData map)
        {
            ...
            for (int i = 0; i < map.Cells.Count; i++)
            {
                ...
                go.transform.position = ToV3(cell.Center);

                var renderer = go.AddComponent<MeshRenderer>();
                var filter = go.AddComponent<MeshFilter>();

                var points = ToLocalV3Space(cell.Edges, cell.Center);
        ...
        private List<Vector3> ToLocalV3Space(Vector2[] edges, Vector2 center)
        {
            List<Vector3> corrected = new List<Vector3>();

            foreach (var point in edges)
                corrected.Add(ToV3(point - center));
